...
- **Payout method mutation auth**: `POST /artist/{artist_id}/payout-method` is ownership-gated with `require_artist_owner` (artist self-service), and remains blocked for impersonation tokens via `require_non_impersonation`.
- **Impersonation precedence rule (mutation routes)**: when both `require_non_impersonation` and ownership/admin dependencies are present, routes declare `require_non_impersonation` first so impersonated Bearer tokens deterministically fail with `403` before resource lookup/ownership checks (avoids `404` short-circuit on missing resources).
